...
mkdir /config/homegear > /dev/null 2>&1
ln -fs /config/homegear /etc/homegear

mkdir -p /share/homegear/lib > /dev/null 2>&1
mkdir -p /share/homegear/log > /dev/null 2>&1
ln -fs /share/homegear/lib /var/lib/homegear
ln -fs /share/homegear/log /var/log/homegear

if ! [ "$(ls -A /etc/homegear)" ]; then
	cp -a /etc/homegear.config/* /etc/homegear/
        chmod -R a+r /etc/homegear/
fi
